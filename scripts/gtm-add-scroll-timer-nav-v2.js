...
(async()=>{
  ...
  const ws = await api(`${BASE}/workspaces`, 'POST', { name: `Scroll Timer Nav v2 ${safeStamp}` });
  const WS = ws.path;
...
(function(){
  ...
  await ensureHtmlTag('Listener - Scroll Timer Internal Nav', listenerHtml, [allPagesTriggerId]);
  ...
  const version = await api(`${WS}:create_version`, 'POST', { name:'Add scroll_50, timer_20_seconds, internal_navigation_click v2' });
