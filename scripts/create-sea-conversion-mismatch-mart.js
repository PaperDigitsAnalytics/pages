...
(async()=>{const raw=JSON.parse(fs.readFileSync(keyFile,'utf8'));const project=raw.project_id;const dataset='marts_sea';const table='sea_campaign_conversion_mismatch';const query=`WITH conv AS (
  SELECT
    f.platform,
    f.account_id,
    f.campaign_id,
...
SELECT
  *,
  SAFE_DIVIDE(primary_conversions_90d, NULLIF(total_conversions_90d,0)) AS primary_conversion_share,
  SAFE_DIVIDE(business_conversions_90d, NULLIF(total_conversions_90d,0)) AS business_conversion_share,
  SAFE_DIVIDE(non_primary_conversions_90d, NULLIF(total_conversions_90d,0)) AS non_primary_conversion_share,
  CASE
    WHEN total_conversions_90d = 0 THEN 'no_conversions'
...
FROM summary
ORDER BY non_primary_conversions_90d DESC, total_conversions_90d DESC, campaign_name`;await upsertView(project,dataset,table,query);})();