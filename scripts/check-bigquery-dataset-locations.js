...
const targets = ['google_ads','bing_ads','facebook_ads','staging_advertising'];
...
(async()=>{const raw=JSON.parse(fs.readFileSync(keyFile,'utf8')); const project=raw.project_id; for(const ds of targets){ const j=await api(`https://bigquery.googleapis.com/bigquery/v2/projects/${project}/datasets/${ds}`); console.log(ds, 'location=', j.location || '(none)'); }})().catch(e=>{console.error(e.stack||String(e)); process.exit(1);});